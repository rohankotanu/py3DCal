...
class Calibrator:
    ...
    def probe(self, home_printer: bool = True, save_images: bool = True, calibration_file_path: Union[str, Path] = None, data_save_path: str = "."):
        """ Executes the probing procedure for a vision-based tactile sensor on 3D printer
    ...
    def probe_reskin(self, home_printer: bool = True, save_data: bool = True, calibration_file_path: Union[str, Path] = None, data_save_path: str = "."):
        ...
        Args:
            home_printer (bool, optional): Determines whether to home the printer prior to probing. Defaults to True.
            save_data (bool, optional): Determines whether sensor data is saved. Defaults to True.
            calibration_file_path (str, optional): The path of the calibration file. For the DIGIT and GelSight Mini,
                if no file is specified, a default calibration file will be used.
            data_save_path (str, optional): The folder in which the data should be saved. If no folder is specified,
                data will be stored in a directory named "sensor_calibration_data" within the current working directory.
        ...
        # Open a csv file to write calibration data
        with open(os.path.join(data_save_path, "probe_data.csv"), 'w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
            csv_writer.writerow(['x_mm', 'y_mm', 'penetration_depth_mm', 'Bx0', 'By0', 'Bz0', 'T0', 'Bx1', 'By1', 'Bz1', 'T1', 'Bx2', 'By2', 'Bz2', 'T2', 'Bx3', 'By3', 'Bz3', 'T3', 'Bx4', 'By4', 'Bz4', 'T4'])

        # Save blank image
        if save_data == True:
            # Open a csv file to write no contact data
            with open(os.path.join(data_save_path, "no_contact_data.csv"), 'w', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow(['x_mm', 'y_mm', 'penetration_depth_mm', 'Bx0', 'By0', 'Bz0', 'T0', 'Bx1', 'By1', 'Bz1', 'T1', 'Bx2', 'By2', 'Bz2', 'T2', 'Bx3', 'By3', 'Bz3', 'T3', 'Bx4', 'By4', 'Bz4', 'T4'])

                for i in range(10):
                    no_contact_data = self.sensor.capture_image()
                    no_contact_data = ['-', '-', '-'] + no_contact_data
                    csv_writer.writerow(no_contact_data)
        ...
        for i in tqdm(range(N), desc="Sensor Calibration Progress"):
            ...
            # If penetration depth does not exceed maximum value, move to calibration point
                ...
                # Take desired number of images
                if save_data == True:
                    with open(os.path.join(data_save_path, "probe_data.csv"), 'a', newline='') as csv_file:
                        csv_writer = csv.writer(csv_file)

                        for _ in range(int(self.calibration_points[i][3])):
                            data = self.sensor.capture_image()
                            data = [self.calibration_points[i][0], self.calibration_points[i][1], self.calibration_points[i][2]] + data
